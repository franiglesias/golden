...
Snapshot testing is a technique in which the output of the subject under test is compared to an existing one (the snapshot) generated by running the very same subject under test. The basic idea is to ensure that changes made to the code doesn't affect to its behaviour.
...
**Current Status**: Verify, approval and golden master mode. Test name customization. Unstable API. 

**Usage advice**: Mostly ready for use, but use it at your own risk until it becomes stable, that will happen when v1.0.0 is reached. Take into account that not all planned features are ready yet. Also, API is pretty stable, but it could change as we are researching the best options.
...
If the snapshot is ok for you, commit it with the code, so it can be used as comparison criteria in future runs. If not, delete the file and run again.
...
We are considering an alternative API approach that could possibly provide better experience.

Pass the option `golden.WaitApproval()`.
...
Once you have the snapshot approved, remove the `golden.WaitApproval()` option. That's all.
...
If the snapshot is ok for you, replace the ToApprove call to Verify, so it can be used as comparison criteria in future runs. If not, modify the code and run it again until the snapshot is fine.

You can use the same options as with `Verify`, like `UseSnapshot`.

### Basic Usage: Golden Master mode

**Warning**: We are considering an alternative API for golden master mode.

Golden Master mode is useful when you want to generate a lot of tests combining different values of the subject under test parameters. It will generate all possible combinations and, it will create a detailed snapshot with all the results. Here is an example testing a `Division` function.

You will need to create a wrapper function that exercises the subject under test managing both parameters and all return values, including errors. Basically, you need to manage to return a `string` representing the outcome of the SUT.
...
Second. I found some limitations in the libraries I was using (Approval tests, mainly) that make my work a bit uncomfortable. So, I started to look for alternatives. I wanted some more flexibility and customization. At the end, I decided to create my own library.
...
In snapshot testing, instead, you first obtain and persist the output of the execution. This is what we call a _snapshot_.
...
Golden automates the process of creating the snapshot the first time that the test runs and using it as criteria in the following runs.
...
In fact, snapshot testing is perfect for testing complex objects or outputs, such as html, xml, json, generated code, etc. Provided that you can create a file with a serialized representation of the response, you can compare the snapshot with subsequent executions of the same unit of code. Suppose you need to generate an API response with lots of data. Instead of trying to figure out how to check every field value, you generate a snapshot with the expected data. After that, you will be using that snapshot to verify the execution. 
...
The technique requieres the creation of a big batch of tests for the same unit of code, introducing combinations of the parameters that you need to pass to such unit. The original Approval Tests library includes Combinations, a library that helps you to generate those combinatorial tests. 

There are several techniques to guess the best values you should use. You can study the code and search for significant values in conditionals, for example, with the help of a graphical coverage tool that shows you what parts of the code are executed or not depending on the values.
...
The signature has 3 parameters, the first two are string and the third is an integer. We want to test the function with several values for each parameter.

#### Wrap the subject under test

Well, the first thing we need is a wrapper function that takes any number of parameters of `any` type and returns something. We are going to pass this function to the `Master` method. 
