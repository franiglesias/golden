...
Snapshot testing is a technique in which the output of the subject under test is compared to a previous one (the snapshot) generated by running the very same subject under test.
...
It consists in the creation of a lot of tests for the same unit of code, introducing combinations of the parameters that you need to pass to such unit. The original Approval Tests library includes Combinations, a library that helps you to generate those combinatorial tests. 
